...
describe("My work", () => {
  ...
  it("makes every workspace row openable", async () => {
    render(await MyWorkPage({ searchParams: Promise.resolve({}) }));
    expect(screen.getByRole("link", { name: /Problem 321 work/iu })).toHaveAttribute("href", "/my-work?workspace=workspace-anchored");
    expect(screen.getByRole("link", { name: /Earlier workspace/iu })).toHaveAttribute("href", "/my-work?workspace=workspace-legacy");
  });
  ...
  it("does not guess a Problem from an unanchored legacy workspace", async () => {
    render(await MyWorkPage({ searchParams: Promise.resolve({ workspace: unanchored.id }) }));
