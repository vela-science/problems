...
export default async function ProblemsPage(props?: { searchParams?: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <PageShell archetype="problem">
    ...
    <PageSection aria-labelledby="published-collections" className="pt-6">
      ...
      <div className="mt-3 overflow-hidden rounded-lg border bg-card"><ul className="divide-y">{collections.map((collection) => <li key={collection.href}><Link href={collection.href} className="vela-object-row group grid gap-4 px-4 py-5 sm:grid-cols-[2.5rem_minmax(0,1fr)_11rem_1.5rem] sm:items-center"><span className="grid size-10 place-items-center rounded-md bg-primary/10 text-primary"><HugeiconsIcon icon={collection.icon} aria-hidden className="size-5" /></span><span className="min-w-0"><span className="block text-title group-hover:text-primary">{collection.name}</span><span className="mt-1 block max-w-3xl text-meta text-muted-foreground">{collection.description}</span></span><span className="text-meta font-medium">{collection.detail}</span><HugeiconsIcon icon={ArrowRight01Icon} aria-hidden className="size-4 text-muted-foreground transition-transform group-hover:translate-x-0.5 group-hover:text-primary" /></Link></li>)}</ul></div>
    </PageSection>
    <PageSection aria-labelledby="problems-to-explore">
      <PageSectionHeader><div><h2 id="problems-to-explore" className="text-title">Problems to explore</h2><p className="mt-1 text-meta text-muted-foreground">Recognizable starting points from both published collections.</p></div><Link href="/search?kind=problem" className="text-meta font-medium text-primary hover:underline">Search all Problems</Link></PageSectionHeader>
      <div className="mt-2 grid gap-6 lg:grid-cols-2">
        <section aria-labelledby="erdos-starting-points"><div className="flex items-center justify-between border-b pb-2"><h3 id="erdos-starting-points" className="text-label">Erdős Problems</h3><Link href="/problems/erdos-problems" className="text-meta text-muted-foreground hover:text-foreground">Browse {erdosCount.toLocaleString()}</Link></div><ul className="divide-y">{previews.map(({ discovery, state }) => <ProblemQuestionRow key={discovery.problem} state={state} number={discovery.problem} collectionLabel="Erdős problem" href={discovery.canonicalPath ?? "/problems/erdos-problems"} />)}</ul></section>
        <section aria-labelledby="fc-starting-points"><div className="flex items-center justify-between border-b pb-2"><h3 id="fc-starting-points" className="text-label">Formal Conjectures</h3><Link href="/problems/formal-conjectures" className="text-meta text-muted-foreground hover:text-foreground">Browse {fcCount}</Link></div><ul className="divide-y">{formalConjecturesCollection.data.items.slice(0, 3).map((item) => <li key={item.route_slug}><Link href={`/problems/formal-conjectures/${item.route_slug}`} className="vela-object-row group flex gap-4 px-2 py-4"><span className="mt-0.5 grid size-8 shrink-0 place-items-center rounded-md bg-primary/10 text-primary"><HugeiconsIcon icon={CodeIcon} aria-hidden className="size-4" /></span><span className="min-w-0 flex-1"><span className="block text-compact font-medium leading-6 group-hover:text-primary"><ScientificText text={item.title} /></span><span className="mt-1 block text-meta text-muted-foreground">{item.source_family} · formalized occurrence</span></span><HugeiconsIcon icon={ArrowRight01Icon} aria-hidden className="mt-1 size-4 shrink-0 text-muted-foreground" /></Link></li>)}</ul></section>
      </div>
    </PageSection>
  </PageShell>;
}
