...
export const metadata: Metadata = {
  ...
  description: "Browse the 1,217 source-owned Erdős problems, then inspect source status, Repository-local State, and exact evidence.",
  ...
  if (view === "overview") {
    ...
    return <PageShell archetype="problem">
      ...
      <PageHero className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_minmax(18rem,.42fr)] lg:items-end">
        <div><p className="text-eyebrow uppercase text-muted-foreground">{corpusLabel}</p><h1 className="mt-3 text-display">Erdős Problems</h1><p className="typeset typeset-compact mt-4 max-w-2xl text-muted-foreground">Browse the source-owned collection by Topic, then inspect each question, its evidence, prior work, and Repository-local State separately.</p><div className="mt-6 flex flex-wrap gap-3"><Button nativeButton={false} render={<Link href={{ pathname: COLLECTION_PATH, query: { view: "all", ...scopeQuery() } }} />}>Open collection directory <HugeiconsIcon icon={ArrowRight} aria-hidden data-icon="inline-end" /></Button><Button nativeButton={false} variant="outline" render={<Link href="/contribute" />}>Add a contribution</Button></div></div>
