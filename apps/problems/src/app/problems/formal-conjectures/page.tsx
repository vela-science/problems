  ...
  return <PageShell archetype="problem">
    ...
    <PageSection aria-labelledby="formal-conjecture-list" className="pt-6">
      <form className="vela-object-surface flex flex-col gap-3 p-3 sm:flex-row sm:items-end" action="/problems/formal-conjectures" method="get">
        <label className="min-w-0 flex-1"><span className="sr-only">Search Formal Conjectures</span><div className="relative"><HugeiconsIcon icon={Search01Icon} aria-hidden className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><Input name="q" defaultValue={q} className="h-11 pl-9" placeholder="Question, declaration, OEIS, or MathOverflow ID…" /></div></label>
        <label className="w-full sm:w-48"><span className="sr-only">Source family</span><Select name="family" defaultValue={family} items={Object.fromEntries([["all", "All source families"], ...families.map((value) => [value, value])])}><SelectTrigger className="h-11 w-full"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">All source families</SelectItem>{families.map((value) => <SelectItem key={value} value={value}>{value}</SelectItem>)}</SelectContent></Select></label>
