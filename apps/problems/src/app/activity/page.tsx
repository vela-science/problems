...
export default async function ActivityPage({ searchParams }: { searchParams: Promise<{ view?: string }> }) {
  ...
  return <PageShell archetype="history">
    <PageHero className="grid gap-7 lg:grid-cols-[minmax(0,1fr)_23rem] lg:items-end"><div><div className="flex items-center gap-2"><HugeiconsIcon icon={Activity01Icon} aria-hidden className="size-5" /><p className="text-eyebrow uppercase text-muted-foreground">Published changes</p></div><h1 className="mt-3 text-display">Updates</h1><p className="typeset typeset-compact mt-4 max-w-2xl text-muted-foreground">See what changed, when, and where. Scientific state changes stay distinct from ordinary code updates; neither counts as acceptance unless a Repository records a Decision.</p></div><p className="text-meta text-muted-foreground"><span className="font-medium text-foreground">Private workspace notes do not appear here.</span> Attempts and drafts remain in My work until they become a published Contribution.</p></PageHero>
