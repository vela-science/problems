...
function nodeSummary(node: NonNullable<GraphResponse>["nodes"][number]) {
  if (!node.label || node.label === node.id || isTechnicalLabel(node.label)) return kindLabel(node.kind);
  return node.label;
}
...
export function RepositoryGraph({ root, initialRepository, repositories, scoped = false }: { root: string; initialRepository: string; repositories: string[]; scoped?: boolean }) {
  ...
  return (
    <div>
      <div className="border-b bg-[var(--vela-surface-sunken)] p-4">
      ...
      <Tabs value={view} onValueChange={(next) => push({ view: next })} className="gap-0">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b bg-[var(--vela-surface-raised)] px-4 py-3"><p className="text-meta text-muted-foreground" aria-live="polite">{data ? view === "map" ? selectedId ? `${data.total.toLocaleString()} items · ${data.edges.length.toLocaleString()} ${lens === "research" ? "exact " : ""}relationships` : `${data.total.toLocaleString()} items · choose one to map` : `${data.total.toLocaleString()} items` : "Loading relationships…"}</p><TabsList><TabsTrigger value="map">Map</TabsTrigger><TabsTrigger value="records">List</TabsTrigger></TabsList></div>
        <TabsContent value="map" className="m-0"><div className="min-h-[34rem] overflow-hidden bg-muted/20">{!selectedId ? <GraphMapChooser records={orderedNodes.slice(0, CHOOSER_LIMIT)} total={data?.total ?? orderedNodes.length} choose={choose} /> : data ? <SigmaMap nodes={data.nodes} edges={data.edges} selected={selected?.id ?? null} onSelect={choose} /> : <div className="grid min-h-[34rem] place-items-center text-body text-muted-foreground">Loading direct relationships…</div>}</div></TabsContent>
      ...
      <Sheet open={Boolean(selected)} onOpenChange={(open) => { if (!open) push({ node: null }); }}>
        {selected ? <SheetContent className="w-full overflow-y-auto sm:max-w-lg"><SheetHeader className="border-b pr-12"><div className="flex min-w-0 flex-wrap items-center gap-2"><SheetTitle className="min-w-0 text-body">{nodeTitle(selected)}</SheetTitle><StatusBadge {...stateBadge(selected.standing, selected.kind)} /></div><SheetDescription className="line-clamp-3">{nodeSummary(selected)}</SheetDescription></SheetHeader><div className="space-y-6 p-4">{data?.neighbors.length ? <section><h3 className="mb-3 text-subtitle">Direct relationships</h3><ItemGroup className="divide-y">{data.neighbors.slice(0, 12).map((neighbor) => <Item key={`${neighbor.relation}:${neighbor.id}`} size="sm" className="min-w-0 rounded-none px-0 py-3" render={<button type="button" onClick={() => choose(neighbor.id)} />}><ItemContent className="min-w-0"><ItemTitle className="line-clamp-1 text-compact">{nodeTitle(neighbor)}</ItemTitle><ItemDescription className="line-clamp-2">{neighbor.relation} · {nodeSummary(neighbor)}</ItemDescription></ItemContent></Item>)}</ItemGroup>{data.neighbor_total > 12 ? <p className="mt-3 text-meta text-muted-foreground">{data.neighbor_total - 12} more direct relationships</p> : null}</section> : null}<Disclosure className="rounded-lg border px-3 py-2 text-meta" summaryClassName="font-medium" summary="Technical identity"><p className="mt-2 text-muted-foreground">{kindLabel(selected.kind)}</p><p className="mt-1 break-all font-mono text-micro">{selected.id}</p></Disclosure>{selected.href ? <Button nativeButton={false} render={<Link href={selected.href} />}>Open {kindLabel(selected.kind)} <HugeiconsIcon icon={ArrowRight} aria-hidden data-icon="inline-end" /></Button> : <p className="text-meta text-muted-foreground">No separate public page.</p>}</div></SheetContent> : null}
      </Sheet>
    </div>
  );
}
...
function GraphRecords({ records, choose }: { records: NonNullable<GraphResponse>["nodes"]; choose: (id: string) => void }) {
  return <><div className="hidden overflow-hidden rounded-lg border md:block"><Table><TableHeader><TableRow><TableHead>Item</TableHead><TableHead className="w-40">Kind</TableHead><TableHead className="w-36">State</TableHead><TableHead className="w-24"><span className="sr-only">Action</span></TableHead></TableRow></TableHeader><TableBody>{records.map((node) => <TableRow key={node.id} className="hover:bg-[var(--vela-surface-hover)]"><TableCell className="max-w-3xl whitespace-normal align-top"><Button type="button" variant="link" onClick={() => choose(node.id)} aria-label={`Inspect ${nodeTitle(node)}`} className="h-auto min-h-0 justify-start p-0 text-left text-body leading-5 text-foreground">{nodeTitle(node)}</Button>{nodeSummary(node) !== nodeTitle(node) ? <p className="mt-1 line-clamp-2 text-meta text-muted-foreground">{nodeSummary(node)}</p> : null}</TableCell><TableCell className="align-top text-meta text-muted-foreground">{kindLabel(node.kind)}</TableCell><TableCell className="align-top"><StatusBadge {...stateBadge(node.standing, node.kind)} /></TableCell><TableCell className="align-top">{node.href ? <Button nativeButton={false} variant="ghost" size="sm" render={<Link href={node.href} />}>Open <HugeiconsIcon icon={ArrowRight} aria-hidden data-icon="inline-end" /></Button> : null}</TableCell></TableRow>)}</TableBody></Table></div><ItemGroup className="divide-y md:hidden">{records.map((node) => <Item key={node.id} className="vela-object-row min-w-0 items-start rounded-md px-2 py-3"><ItemContent className="min-w-0"><Button type="button" variant="link" onClick={() => choose(node.id)} aria-label={`Inspect ${nodeTitle(node)}`} className="h-auto min-h-0 min-w-0 justify-start p-0 text-left"><ItemTitle className="line-clamp-1 text-compact leading-5">{nodeTitle(node)}</ItemTitle></Button><ItemDescription className="line-clamp-2">{nodeSummary(node) !== nodeTitle(node) ? nodeSummary(node) : kindLabel(node.kind)}</ItemDescription></ItemContent><StatusBadge {...stateBadge(node.standing, node.kind)} /></Item>)}</ItemGroup></>;
}

/* The chooser showed ten rows out of a list capped at 24, under a heading that
   counted the unfiltered total — three numbers, no two of which agreed. One cap
   now, applied by the caller and named here, with the list scrolling inside the
   panel and the remainder pointing at the tab that paginates. */
