...
function GraphMapChooser({ records, choose }: { records: NonNullable<GraphResponse>["nodes"]; choose: (id: string) => void }) {
  return <div className="grid min-h-[34rem] md:grid-cols-[24rem_minmax(0,1fr)]"><div className="border-b bg-[var(--vela-surface-sunken)] p-4 md:border-b-0 md:border-r"><h2 className="text-subtitle">Choose an item</h2><ItemGroup className="mt-3 gap-1">{records.slice(0, 10).map((node) => <Item key={node.id} size="sm" className="vela-object-row min-w-0 rounded-md px-2 py-3" render={<button type="button" onClick={() => choose(node.id)} />}><ItemContent className="min-w-0"><ItemTitle className="line-clamp-1 text-compact">{nodeTitle(node)}</ItemTitle><ItemDescription className="line-clamp-2">{nodeSummary(node) !== nodeTitle(node) ? nodeSummary(node) : kindLabel(node.kind)}</ItemDescription></ItemContent><StatusBadge {...stateBadge(node.standing, node.kind)} /></Item>)}</ItemGroup></div><div className="grid min-h-72 place-items-center bg-[var(--vela-surface-sunken)] p-8 text-center"><div><HugeiconsIcon icon={GitFork} aria-hidden className="mx-auto size-9 text-primary" /><h2 className="mt-4 text-title">Open an exact neighbourhood</h2><p className="mt-2 max-w-md text-compact text-muted-foreground">Select a Problem, Result, source, or check. The map draws only retained relationships.</p></div></div></div>;
}
