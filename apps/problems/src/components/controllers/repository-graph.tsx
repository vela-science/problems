...
function nodeTitle(node: NonNullable<GraphResponse>["nodes"][number]) {
  ...
  if (node.kind === "claim") return `${state || "Research"} Contribution`;
...
function GraphMapChooser({ records, choose }: { records: NonNullable<GraphResponse>["nodes"]; choose: (id: string) => void }) {
  return <div className="grid min-h-[34rem] md:grid-cols-[24rem_minmax(0,1fr)]"><div className="border-b bg-background p-4 md:border-b-0 md:border-r"><h2 className="text-subtitle">Choose an item</h2><ItemGroup className="mt-3 divide-y">{records.slice(0, 10).map((node) => <Item key={node.id} size="sm" className="min-w-0 rounded-none px-0 py-3" render={<button type="button" onClick={() => choose(node.id)} />}><ItemContent className="min-w-0"><ItemTitle className="line-clamp-1 text-compact">{nodeTitle(node)}</ItemTitle><ItemDescription className="line-clamp-2">{node.label !== nodeTitle(node) ? node.label : kindLabel(node.kind)}</ItemDescription></ItemContent><StatusBadge {...stateBadge(node.standing, node.kind)} /></Item>)}</ItemGroup></div><div className="grid min-h-72 place-items-center p-8 text-center"><div><HugeiconsIcon icon={GitFork} aria-hidden className="mx-auto size-8 text-muted-foreground" /><h2 className="mt-4 text-title">Open an exact neighbourhood</h2><p className="mt-2 max-w-md text-compact text-muted-foreground">Select a Problem, Contribution, source, or check. The map draws only retained relationships.</p></div></div></div>;
}
