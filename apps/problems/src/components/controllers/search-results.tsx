...
export function SearchResults({ projectionRoot, repositories }: { projectionRoot: string; repositories: string[] }) {
  ...
  return <Command shouldFilter={false} className="rounded-lg border bg-card p-0">
    <div className="border-b p-3 sm:p-4">
      <CommandInput value={query} onValueChange={(value) => replace({ q: value || null })} placeholder="Claim, problem, proposal, source…" aria-label="Search published records" />
    ...
    <CommandList className="max-h-[62vh] p-1">
      ...
      {!hasIntent ? <Empty className="min-h-56 border-0"><EmptyHeader><EmptyMedia variant="icon"><HugeiconsIcon icon={Search}  /></EmptyMedia><EmptyTitle>Find a published record</EmptyTitle><EmptyDescription>Enter an exact ID, assertion, source, or choose a repository, kind, or state.</EmptyDescription></EmptyHeader></Empty> : null}
