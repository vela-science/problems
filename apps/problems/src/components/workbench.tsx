...
export async function Workbench({ state, hostedAccount, selectedWorkspace }: { state: State; hostedAccount: HostedAccount | null; selectedWorkspace?: string }) {
  ...
  return <div className="mt-8 space-y-10">
    ...
    {activity.anchors.length ? <section aria-labelledby="freshness-heading"><h2 id="freshness-heading" className="text-title">Anchor freshness</h2><ul className="mt-4 grid gap-3 sm:grid-cols-2">{activity.anchors.map((anchor) => { const freshness = assessAnchorFreshness(anchor, currentAnchor); return <li key={anchor.root} className="border p-4"><div className="flex items-center justify-between gap-3"><span className="font-mono text-micro break-all">{anchor.root.slice(0, 24)}…</span><Badge variant={freshness.state === "current" ? "default" : "secondary"}>{freshness.state.replaceAll("_", " ")}</Badge></div>{"fields" in freshness ? <p className="mt-2 text-meta text-muted-foreground">Changed: {freshness.fields.join(", ")}</p> : null}</li>; })}</ul></section> : null}

    <section aria-labelledby="approaches-heading" className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_22rem]">
      <div><div className="flex items-end justify-between gap-4"><div><p className="text-eyebrow uppercase text-muted-foreground">Direction</p><h2 id="approaches-heading" className="mt-1 text-title">Approaches and forks</h2></div><span className="font-mono text-meta text-muted-foreground">{activity.approaches.length}</span></div>
        {activity.approaches.length ? <ol className="mt-4 divide-y border-y">{activity.approaches.map((approach) => { const id = string(approach, "id", "approach_id"); const version = number(approach, "version"); return <li key={id} className="py-5"><div className="flex flex-wrap items-center gap-2"><h3 className="text-subtitle">{string(approach, "title")}</h3><Badge variant="outline">v{version}</Badge>{string(approach, "parentApproachId", "parent_approach_id") ? <Badge variant="secondary">fork</Badge> : null}</div><p className="mt-2 text-body text-muted-foreground">{string(approach, "summary")}</p><form action={forkApproachAction} className="mt-3 flex flex-wrap gap-2"><ScopeFields scope={scope} /><input type="hidden" name="approachId" value={id} /><input type="hidden" name="expectedVersion" value={version} /><Input className="max-w-64" name="title" aria-label="Fork title" placeholder="Optional fork title" /><Button type="submit" size="sm" variant="outline">Fork approach</Button></form>
          <form action={createAttemptAction} className="mt-4 grid gap-3 border-l-2 pl-4 sm:grid-cols-2"><ScopeFields scope={scope} /><input type="hidden" name="approachId" value={id} /><FormField label="Attempt title" name="title" placeholder="Test the finite case" /><FormField label="Provider" name="provider" placeholder="human, Codex, local runner…" /><FormField label="Provider session reference" name="externalSessionId" placeholder="Optional external ID" required={false} /><FormField label="External locator" name="locator" placeholder="Optional https:// or local locator" required={false} /><Button className="w-fit sm:col-span-2" type="submit" size="sm">Start planned attempt</Button></form></li>; })}</ol> : <p className="mt-4 border border-dashed p-5 text-body text-muted-foreground">No approach yet. Name a direction before starting an attempt.</p>}
      </div>
      <form action={createApproachAction} className="h-fit space-y-4 border p-5"><ScopeFields scope={scope} /><div><p className="text-eyebrow uppercase text-muted-foreground">New approach</p><p className="mt-1 text-meta text-muted-foreground">Bound to the exact Problem anchor above.</p></div><FormField label="Title" name="title" placeholder="Reduce to a finite obstruction" /><div className="grid gap-1.5"><Label htmlFor="approach-summary">Summary</Label><Textarea id="approach-summary" name="summary" required placeholder="What direction will this explore, and what would falsify it?" /></div><Button type="submit">Create approach</Button></form>
    </section>

    <section aria-labelledby="attempts-heading"><div className="flex items-end justify-between"><div><p className="text-eyebrow uppercase text-muted-foreground">Execution</p><h2 id="attempts-heading" className="mt-1 text-title">Attempts</h2></div><span className="font-mono text-meta text-muted-foreground">{activity.attempts.length}</span></div>
    ...
    <section className="grid gap-6 lg:grid-cols-2">
      <div><form action={addDiscussionAction} className="space-y-4 border p-5"><ScopeFields scope={scope} /><div><p className="text-eyebrow uppercase text-muted-foreground">Note or comment</p><h2 className="mt-1 text-subtitle">Record reasoning in context</h2></div><div className="grid grid-cols-2 gap-3"><label className="grid gap-1 text-meta">Kind<select name="kind" className="h-11 border bg-background px-3"><option value="note">Note</option><option value="comment">Comment</option></select></label><label className="grid gap-1 text-meta">Visibility<select name="visibility" className="h-11 border bg-background px-3"><option value="workspace">Workspace</option><option value="private">Private</option></select></label></div><Textarea name="body" required placeholder="What changed your view of this Problem?" /><Button type="submit">Add entry</Button><p className="text-meta text-muted-foreground">Private notes are visible only to their author; every write still receives an append-only activity audit entry.</p></form>
