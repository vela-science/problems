  ...
  return <header className="sticky top-0 z-40 border-b bg-background/95 backdrop-blur supports-backdrop-filter:bg-background/80">
    <div className="mx-auto flex h-14 max-w-[90rem] items-center gap-4 px-4 sm:px-6">
      <Link href="/" className="flex min-h-11 items-center gap-2 font-semibold tracking-tight">
        <span aria-hidden className="grid size-7 place-items-center border border-foreground bg-foreground text-[11px] font-bold text-background">V</span>
        <span>Problems</span>
      </Link>
      <Badge variant="outline" className="hidden sm:inline-flex">non-authoritative workbench</Badge>
      <nav aria-label="Product surfaces" className="ml-auto flex items-center gap-1 text-meta">
        <Button nativeButton={false} variant="ghost" size="sm" render={<a href="https://app.vela.space/repositories" />}>Observatory</Button>
        ...
        </> : authAvailable ? <Button nativeButton={false} size="sm" render={<Link href="/sign-in" prefetch={false} />}>Sign in to work</Button> : null}
      </nav>
    </div>
  </header>;
}
