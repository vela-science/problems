  ...
  return <div>
    <div className="not-typeset relative mx-auto hidden aspect-square w-full max-w-xl sm:block" data-not-typeset>
      ...
      {visible.map((problem, index) => {
        ...
        return <Link key={`${problem.repository}/${problem.problem}`} href={problem.canonicalPath ?? "/problems"} style={{ left: `${x}%`, top: `${y}%` }} className="absolute flex size-16 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full border bg-background font-mono text-label shadow-sm transition-[transform,background-color] duration-200 hover:scale-105 hover:bg-muted focus-visible:outline-2 focus-visible:outline-offset-4">{problem.problem}</Link>;
      })}
    </div>
    <ol className="grid gap-2 sm:hidden">{visible.map((problem) => <li key={`${problem.repository}/${problem.problem}`}><Link href={problem.canonicalPath ?? "/problems"} className="flex items-center gap-4 rounded-lg bg-muted/30 px-3 py-3.5 transition-colors duration-200 hover:bg-muted/50 focus-visible:outline-2 focus-visible:outline-offset-2"><span className="font-mono text-title">{problem.problem}</span><span className="min-w-0 flex-1 truncate text-label">{problem.theme}</span><HugeiconsIcon icon={ArrowRight} aria-hidden className="size-4" /></Link></li>)}</ol>
  </div>;
}
