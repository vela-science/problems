...
export function workspaceObjects({ state, activity, workspace, scope, currentAnchorRoot, basePath }: { state: State; activity: ProblemActivity; workspace: Workspace; scope: Scope; currentAnchorRoot: string; basePath: string }): WorkspaceObject[] {
  ...
  for (const draft of activity.drafts) {
    const id = draft.id;
    objects.push({ id: `draft:${id}`, recordId: id, parentId: null, group: "outputs", kind: "draft", label: "Unsigned Result draft", summary: "Schema-validated payload bytes ready for a compatible local tool.", meta: `Problem-scoped · ${anchorMeta(draft)}`, version: draft.version, anchorRoot: draft.anchorRoot, content: <div><div className="flex flex-wrap items-center gap-2"><h2 className="text-title">Unsigned Result draft</h2><Badge variant="secondary">unsigned</Badge></div><p className="mt-3 max-w-3xl text-body text-muted-foreground">Download the validated draft, then continue inside the source Repository. Hosted Problems cannot sign or submit it.</p><Button className="mt-5" nativeButton={false} variant="outline" render={<Link href={`/drafts/${id}/export?workspace=${workspace.id}`} />}>Download unsigned draft</Button><ol aria-label="Result handoff" className="mt-7 grid gap-0 overflow-hidden rounded-lg border sm:grid-cols-4 sm:divide-x">
