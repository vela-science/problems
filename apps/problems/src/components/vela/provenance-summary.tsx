 ...
 * record-tier and live at ?view=record.
 *
...
export function ProvenanceSummary({ state, basePath }: { state: State; basePath: string }) {
  ...
  const recordHref = `${basePath}?view=record`;
  const audit = state.sourceAudits.length
    ? <>{" "}The source publishes <Link href={`${basePath}?view=sources`} className="underline underline-offset-4">its own audit</Link>.</>
    : null;
  if (current) {
