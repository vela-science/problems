...
/* One typed relationship between two retained frontier objects, with the exact
   basis class that justifies stating it. An edge never changes Standing: the
   strongest basis here, authority_decided, names a Decision someone already
   made, and the weakest, heuristic_advisory, must say in `nonclaims` what it
   does not establish. `basis_ref` names the retained record the basis rests on
   — a native record, a decision event, a semantic delta — so a reader can walk
   from any edge back to bytes. `problem_entity_id` is a navigation grouping key
   from the reviewed resolver, not a second identity claim. */
