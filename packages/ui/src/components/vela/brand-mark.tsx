...
export function BrandMark({
  ...
  size?: number;
  className?: string;
  /* Callers set `--vela-mark-accent` here to separate the sail from its
     waterline. Where both resolve to one colour the mark reads as a blob at
     small sizes, which is every size it is used at inside a row. */
  ...
  return <svg viewBox={profile === "micro" ? "0 0 256 256" : "0 0 1000 800"} fill="none" aria-hidden focusable="false" className={cn("block flex-none overflow-visible", className)} style={{ width: size, height, ...style }}>
    {profile === "micro" ? <>
      ...
      <path fill="var(--vela-mark-accent, var(--direction))" d="M20 205 C91 204 163 202 234 211 L232 220 C164 212 91 213 21 216 Z" />
    </> : <>
      ...
      <path stroke="var(--vela-mark-accent, var(--direction))" strokeLinecap="round" strokeWidth="12" d="M65 657 C320 651 585 623 915 670" />
    </>}
  </svg>;
}
