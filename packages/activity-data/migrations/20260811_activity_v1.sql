...
CREATE TABLE IF NOT EXISTS activity.schema_migrations (
...
CREATE TABLE IF NOT EXISTS activity.attempts (
  ...
  external_session_id text CHECK (external_session_id IS NULL OR length(external_session_id) BETWEEN 1 AND 1000),
...
CREATE TABLE IF NOT EXISTS activity.work_requests (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  workspace_id uuid NOT NULL REFERENCES activity.workspaces(id) ON DELETE CASCADE,
  anchor_root text NOT NULL,
  approach_id uuid,
  attempt_id uuid,
  created_by_account_id uuid NOT NULL REFERENCES activity.accounts(id),
  assignee_account_id uuid REFERENCES activity.accounts(id),
  kind text NOT NULL CHECK (kind IN ('assignment', 'reproduction')),
  state text NOT NULL DEFAULT 'open' CHECK (state IN ('open', 'claimed', 'completed', 'cancelled')),
  title text NOT NULL CHECK (length(btrim(title)) BETWEEN 1 AND 300),
  detail text NOT NULL CHECK (length(btrim(detail)) BETWEEN 1 AND 16000),
  version bigint NOT NULL DEFAULT 1 CHECK (version > 0),
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now(),
  FOREIGN KEY (workspace_id, anchor_root)
...
CREATE TABLE IF NOT EXISTS activity.activity_audit_entries (
  ...
  operation text NOT NULL CHECK (operation IN (
    ...
    'attempt.create', 'attempt.update', 'discussion.add', 'work_request.create',
    'artifact.attach', 'submission_draft.save'
  )),
  subject_kind text NOT NULL CHECK (subject_kind IN (
    'workspace', 'follow', 'approach', 'attempt', 'discussion',
    'work_request', 'artifact_ref', 'submission_draft'
  )),
...
CREATE INDEX IF NOT EXISTS activity_work_requests_problem_idx
  ON activity.work_requests (workspace_id, anchor_root, state, created_at DESC);
...
SET search_path = pg_catalog, activity
AS $function$
  SELECT coalesce(jsonb_agg(to_jsonb(w) || jsonb_build_object('role', m.role) ORDER BY w.name, w.id), '[]'::jsonb)
...
CREATE OR REPLACE FUNCTION activity_api.get_problem_activity(
  p_account_id uuid, p_workspace_id uuid, p_repository_id text, p_problem_id text
)
RETURNS jsonb
LANGUAGE plpgsql
STABLE
...
BEGIN
  ...
  WITH anchors AS (
    ...
    WHERE workspace_id = p_workspace_id AND repository_id = p_repository_id AND problem_id = p_problem_id
  )
...
GRANT SELECT ON activity.schema_migrations TO vela_activity_migrator;
