...
const [matrix] = await admin.query(`SELECT
  ...
  has_database_privilege('vela_activity_app', 'vela_projection', 'CONNECT') AS problems_connect,
  has_database_privilege($1, 'vela_activity', 'CONNECT') AS projection_reader_activity_connect,
  ...
  ), 'EXECUTE') AS command_execute`, [projectionReaderIdentity.loginRole]);
if (!matrix?.activity_connect || matrix.activity_temp || matrix.migrator_temp || matrix.problems_connect || !matrix.api_usage
  || matrix.projection_reader_activity_connect || matrix.storage_usage
