...
const [matrix] = await admin.query(`SELECT
  ...
  ), 'EXECUTE') AS command_execute`);
if (!matrix?.activity_connect || matrix.observatory_connect || !matrix.api_usage
  || matrix.storage_usage || matrix.base_access || !matrix.command_execute) {
