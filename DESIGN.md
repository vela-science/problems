...
**Masthead:** the sail mark alone, top-left, returning to /vela from every page. No wordmark, no text links, no menu, no sticky bar: a lockup with a mono wordmark and one contextual text link was built and rejected the same day (2026-07-16) — the glyph-only mark is the decided register, and text in the masthead should not be re-proposed. Static and script-free on both registers; the landing's night variant re-inks the row through `.bl-mast`. Use the full two-lineage construction reference at 32px and above and a hand-simplified sail for the favicon. Do not invent alternate route marks for publication surfaces.
