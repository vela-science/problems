...
**Masthead:** a publisher's imprint, not a navbar. The sail-and-wordmark lockup sits top-left on every page and always returns to /vela; the wordmark is set in the mono label voice until the custom-drawn VELA artwork from the brand handoff exists. The right slot holds at most one link, and always to the other place: every page offers Constellations except the essay itself, where the slot stays empty (a self-link is noise, and /vela already belongs to the lockup). Static and script-free on both registers; the landing's night variant re-inks the whole row through `.bl-mast`. Two destinations means no menu, no hamburger, no sticky bar at any width. Use the full two-lineage construction reference at 32px and above and a hand-simplified sail for the favicon. Do not invent alternate route marks for publication surfaces.
