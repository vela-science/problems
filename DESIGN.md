...
**Landing (one sky):** the /vela landing is a single day-night cycle, not alternating blocks. The hero's night dawns into the paper premise, dusk falls for the voyage, dawn returns for the entry paths and evidence, and night closes the page. Register changes travel through `.vl-band` gradient strips — a long oklab ramp that eases out of one register and commits to the next well before the edge, with a soft-gold bloom at each band's horizon and a few stars crossing on the night side. The paper hours keep a whisper of the same sky (indigo high in one corner, gold low in the other, a few percent over paper). The lane is the observatory ledger: star-atlas instrument on the house's paper — never neon dev-tool. The hero's snapshot row reads as the horizon instrument (mono-caps labels, tabular numerals, gold ticks), the last thing the night shows before dawn.
