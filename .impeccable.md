...
`impeccable detect src` currently reports `single-font` against `src/layouts/Base.astro`. Treat that as a false positive unless the font link or token layer changes: the site deliberately uses Cormorant Garamond for display, EB Garamond for body, Inter Tight for UI labels, and JetBrains Mono for code/instrument labels.
