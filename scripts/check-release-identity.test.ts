...
describe("one Web release identity", () => {
  test("accepts one version across every private workspace", () => {
    expect(checkReleaseIdentity(repository)).toMatchObject({ version: "0.421.0-rc.1", tag: "v0.421.0-rc.1" });
  });
  ...
  test("rejects an independently versioned internal package", () => {
    ...
    expect(() => checkReleaseIdentity(root)).toThrow("differs from root 0.421.0-rc.1");
  });
  ...
  test("requires the advertised production tag to resolve to the exact build commit", () => {
    ...
    expect(() => checkReleaseIdentity(root, "v0.421.0-rc.1")).toThrow();

    git(root, "-c", "tag.gpgSign=false", "tag", "v0.421.0-rc.1");
    expect(checkReleaseIdentity(root, "v0.421.0-rc.1")).toMatchObject({
      version: "0.421.0-rc.1",
      tag: "v0.421.0-rc.1",
    ...
    expect(() => checkReleaseIdentity(root, "v0.421.0-rc.1")).toThrow("not HEAD");
  });
  ...
  test("accepts a detached production builder only with the exact registered commit", () => {
    ...
    expect(
      checkReleaseIdentity(root, "v0.421.0-rc.1", commit, true),
    ).toMatchObject({ tag: "v0.421.0-rc.1", commit });
    expect(() =>
      checkReleaseIdentity(root, "v0.421.0-rc.1", `${"0".repeat(40)}`, false)
    ).toThrow();
  });
});
