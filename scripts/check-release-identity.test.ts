...
describe("one Web release identity", () => {
  test("accepts one version across every private workspace", () => {
    expect(checkReleaseIdentity(repository)).toMatchObject({ version: "0.420.5", tag: "v0.420.5" });
  });
  ...
  test("rejects an independently versioned internal package", () => {
    ...
    expect(() => checkReleaseIdentity(root)).toThrow("differs from root 0.420.5");
  });
  ...
  test("requires the advertised production tag to resolve to the exact build commit", () => {
    ...
    expect(() => checkReleaseIdentity(root, "v0.420.5")).toThrow();

    git(root, "-c", "tag.gpgSign=false", "tag", "v0.420.5");
    expect(checkReleaseIdentity(root, "v0.420.5")).toMatchObject({
      version: "0.420.5",
      tag: "v0.420.5",
    ...
    expect(() => checkReleaseIdentity(root, "v0.420.5")).toThrow("not HEAD");
  });
  ...
  test("accepts a detached production builder only with the exact registered commit", () => {
    ...
    expect(
      checkReleaseIdentity(root, "v0.420.5", commit, true),
    ).toMatchObject({ tag: "v0.420.5", commit });
    expect(() =>
      checkReleaseIdentity(root, "v0.420.5", `${"0".repeat(40)}`, false)
    ).toThrow();
  });
});
