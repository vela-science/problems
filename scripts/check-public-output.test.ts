...
function fixture(entries: Record<string, string> = {}) {
  ...
  const defaults = [
    ...
    "apps/problems/.next/server/app/index.rsc",
  ];
...
describe("public output secret and privacy scan", () => {
  test("accepts delivered output without server or account material", () => {
    expect(scanPublicOutput(fixture())).toMatchObject({
      ok: true,
      profiles: ["observatory", "problems", "www"],
    });
  });
  ...
  test("rejects private hosted-account data from Problems output", () => {
    const root = fixture({
      "apps/problems/.next/server/app/index.rsc": "user_01ABCDEF234567 researcher@example.test",
    });
