...
/* Two guarantees that used to be asserted in @vela/brand against a semantic
   ...
   focusable elements. */
/* One object, one page layout. A route that varies `layout` across sections of
   the same object varies the page maximum underneath that object's own header,
   which then resizes as a reader moves between its tabs — measured at 176px of
   width and 88px of offset before this was one value. */
const problemPageSource = readFileSync(join(root, "apps/problems/src/components/vela/problem-page.tsx"), "utf8");
if (/layout=\{[^}]*\?/u.test(problemPageSource)) {
  failures.push("the Problem shell must not vary its layout by section: the header is shared across all five");
}
